...
public class GeneralTree<T>{
	...
	public boolean esAncestro(T a, T b) {
		return (!this.isEmpty()) ? this.tieneAncestro(a, b, false) : false;
	}
	private boolean tieneAncestro(T a, T b, boolean ancestroEncontrado) {
		boolean tiene = true;
		boolean ancestro = ancestroEncontrado;
		
		//Primero preguntar si el valor del nodo actual es igual al valor 'a'
		if (this.getData() == a)
			ancestro = true;
		
		//Luego preguntar por el caso en el que encontramos a 'b' antes que 'a'
		if (!ancestro && this.getData() == b)
			tiene = false;
		
		//Revisar los demás casos en los que ni se haya encontrado el ancestro o aún no se haya encontrado 'b'
		if (!ancestro || this.getData() != b) {
			if (!this.isLeaf()) {
				for (GeneralTree<T> hijo : this.getChildren())
					tiene = tiene & hijo.tieneAncestro(a, b, ancestro);
			}
		}

...
public class PersonajeTest {
	...
	void test_inicializacion() {
		...
		Assertions.assertEquals(2, circe.getInteligencia());
		
		Assertions.assertEquals(1, nadie.getNivel());
		Assertions.assertEquals(2, nadie.getFuerza());
	}
	...
	void test_subirNivel_humano() {
		...
		double valorViejo = circe.getInteligencia();
		// Subirle el nivel 5 veces para que sea 6 (valor de borde)
		for (int i = 0; i < 5; i++) { circe.subirNivel(); }
		Assertions.assertEquals(valorViejo, circe.getInteligencia());
		...
		Assertions.assertEquals(1 + (7 / 6), odiseo.getFuerza());
	}
	...
	void test_subirNivel_orco() {
		...
		double valorViejo = nadie.getFuerza();
		nadie.subirNivel(); // Nivel = 2
		Assertions.assertEquals(valorViejo, nadie.getFuerza());
		
		// PARTICION NO.2: Nivel múltiplo de 3 (se modifican las habilidades) (rol mago)
		
		polifemo.subirNivel();
		polifemo.subirNivel(); // Nivel = 3
		Assertions.assertEquals(1 + (3/2 * 3), polifemo.getInteligencia());
	}
}
